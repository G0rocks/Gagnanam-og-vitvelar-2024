...
def mvn_basis(
    ...
    * design_matrix - [NxM] is the basis function vectors containing a basis function
    ...
    for i in range(M):
        ...
        # Calculate the multivariate normal distribution for the current mean and the isotropic covariance matrix
...
def _plot_mvn(design_matrix):
    ...
    Input:
    design_matrix: (N x M) capital phi matrix
    '''
    # Find M and M
...
def max_likelihood_linreg(
    ...
    * M:    Added by Huldar. Seems that the gradescope gives me an error for not having it so I added it.
    ...
    w_ML = (torch.inverse(reg_matrix + design_matrix_T@design_matrix)@design_matrix_T)*targets
